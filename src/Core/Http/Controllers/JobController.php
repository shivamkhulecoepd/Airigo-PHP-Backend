...
class JobController extends BaseController
{
    ...
    /**
     * Upload company logo image (for new jobs or existing jobs)
     * This endpoint allows uploading an image without requiring a job ID,
     * making it suitable for pre-upload before job creation.
     */
    public function uploadImage(ServerRequestInterface $request)
    {
        $user = $this->getUser($request);
        if (!$user) {
            return ResponseBuilder::unauthorized(['message' => 'User not authenticated']);
        ...
        try {
            ...
            if (!isset($uploadedFiles['image']) || $uploadedFiles['image']->getError() !== UPLOAD_ERR_OK) {
                return ResponseBuilder::badRequest(['message' => 'Image file is required']);
            }
            ...
            $uniqueFilename = 'company_logo_' . time() . '_' . bin2hex(random_bytes(8)) . '.' . $extension;
            
            // Upload to Firebase Storage
            $fileUrl = $this->firebaseStorage->uploadFile($tempPath, $uniqueFilename);

            if (!$fileUrl) {
                return ResponseBuilder::serverError(['message' => 'Failed to upload image to storage']);
            }
